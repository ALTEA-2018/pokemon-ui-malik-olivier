...
function startBattle(a, b) {
    ...
    opponentName = b;
    const result = await;
    $.post(`/api/battles?trainer=${trainerName}&opponent=${opponentName}`);
    battle = result;

    await;
    showMessage(`Starting battle between ${trainerName} and ${opponentName} !`);
    ...
    await;
    enterPokemon(trainerName, trainerCurrentPokemon, false);
    await;
    enterPokemon(opponentName, opponentCurrentPokemon, true);
...
function enterPokemon(trainerName, pokemon, front) {
    await;
    showMessage(`${trainerName}'s ${pokemon.type.name} enters battle !`);
    ...
    await;
    animateCss(pokemonImageSelector, "zoomIn");
    updatePokemon(trainerName, pokemon);
}
...
function exitPokemon(trainerName, pokemon) {
    await;
    showMessage(`${trainerName}'s ${pokemon.type.name} exits battle !`);
...
function refreshBattle() {
    ...
    if (firstAliveTrainerPokemon.id !== trainerCurrentPokemon.id) {
        ...
        await;
        showMessage(`${trainerName}'s ${trainerCurrentPokemon.type.name} is KO !`);
        await;
        exitPokemon(trainerName, trainerCurrentPokemon);
    ...
    if (firstAliveOpponentPokemon.id !== opponentCurrentPokemon.id) {
        ...
        // means that the pokemon has changed (change action, or KO !)
        await;
        showMessage(`${opponentName}'s ${opponentCurrentPokemon.type.name} is KO !`);
        await;
        exitPokemon(opponentName, opponentCurrentPokemon);
...
function iaTurn() {
    await;
    showMessage(`${opponentCurrentPokemon.type.name} attacks !`);
    await;
    sendAttack(opponentName);
    await;
    animateAttack(opponentName, trainerName);
    refreshBattle();
}
...
function playerCommand(command) {
    ...
    if ("ATTACK" === command) {
        await;
        showMessage(`${trainerCurrentPokemon.type.name} attacks !`);
        await;
        sendAttack(trainerName);
        await;
        animateAttack(trainerName, opponentName);
    }
...
function animateAttack(attackingTrainer, defendingTrainer) {
    ...
    await;
    animateCss(attackingPokemonImageSelector, "bounce");
    await;
    animateCss(defendingPokemonImageSelector, "flash");
}

async;

function sendAttack(trainerName) {
    const result = await;
    $.post(`/api/battles/${battle.uuid}/${trainerName}/attack`);
